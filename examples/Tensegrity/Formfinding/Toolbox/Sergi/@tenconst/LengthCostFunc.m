function cost_func=LengthCostFunc(tc)
...
for i=1:1:tc.tensegrity.num_edges
    if strcmp(tc.tensegrity.edges(i).rest_length.type,'fixed')
        cost_func=cost_func+0.5*tc.tensegrity.edges(i).stiffness.value*(tc.tensegrity.edges(i).length.var-tc.tensegrity.edges(i).rest_length.value)*(tc.tensegrity.edges(i).length.var-tc.tensegrity.edges(i).rest_length.value);
    else
        cost_func=cost_func+0.5*tc.tensegrity.edges(i).stiffness.value*(tc.tensegrity.edges(i).length.var-tc.tensegrity.edges(i).rest_length.var)*(tc.tensegrity.edges(i).length.var-tc.tensegrity.edges(i).rest_length.var);        
    end
