function [te,file]=loadtensegrity(te,file)
...
try
    ...
    [te.tenconst.tensegrity.dimension,file]=readfieldint(file,DimTxt);
    % read the class of the tensegrity structure
    [te.tenconst.tensegrity.class,file]=readfieldint(file,ClassTxt);
    ...
    for num=1:1:num_nodes
        ...
        te.tenconst.tensegrity=addnode(te.tenconst.tensegrity,n);
    end
    % read the information of each edge
    for num=1:1:num_edges
        e=edgeenv();
        [e,file]=readedgeinfo(file,e);
        te.tenconst.tensegrity=addedge(te.tenconst.tensegrity,e);
    end
    te.tenconst.tensegrity=setfilename(te.tenconst.tensegrity,file.filename);    
catch
    rethrow(lasterror);
end
