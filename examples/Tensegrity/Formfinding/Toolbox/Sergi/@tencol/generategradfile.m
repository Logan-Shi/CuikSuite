function to=generategradfile(to,path,filename)
...
new_func=subs(to.tenopt.gradient,[old_par,old_opt],[new_par,new_opt]);
...
for i=0:1:size(to.tenopt.gradient,1)-1
    text1=['[',num2str(i),'][0]'];
    text2=['[',num2str(i),']'];
...
fclose(file);
to.tenopt.grad_file=strtok(filename,'.');
