function plotEllipsoid(A, C, fname, varargin)
%
%  plotEllipsoid(A,C,N) plots a 2D ellipse or a 3D ellipsoid 
...
elseif strcmp(Type,'3D'),
    ...
    %  rotate and translate the ellipsoid to the actual center point
    %---------------------------------------------------------------
    XX = zeros(N+1,N+1);
    YY = zeros(N+1,N+1);
    ZZ = zeros(N+1,N+1);
...
%    end
    ...
    F = fopen([fname '_ellipse.off'],'w');
    fprintf(F,'%s\n','VECT');
    fprintf(F,'%s%d%s\n','1 ',length(X)*length(X),' 0');
    fprintf(F,'%d\n',length(X)*length(X));
    fprintf(F,'%s\n','0');
    for k = 1:length(X),
        for j = 1:length(X),
            fprintf(F,'%f%s%f%s%f\n',XX(k,j),' ',YY(k,j),' ',ZZ(k,j));
        end
    end
end
