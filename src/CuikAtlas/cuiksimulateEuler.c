...
/** \file cuiksimulateEuler.c

    \brief Main body of the \ref cuiksimulateEuler.c "cuiksimulateEuler" application.
    ...
    digraph applications {
      ...
      cuiksimulateEuler    [label="cuiksimulateEuler"  color="#99ff99" shape=ellipse];
      ...
      param -> cuiksimulateEuler
      world -> cuiksimulateEuler
      samples -> cuiksimulateEuler
      cuiksimulateEuler -> atlas;
      cuiksimulateEuler -> trajectory;
    }
    \enddot
    </center>
...
/** 
   ...
   Main body of the \ref cuiksimulateEuler.c "cuiksimulateEuler" application.

   Simulates the dynamical effects of an action on a given state.

   Use:
       - \b cuiksimulateEuler \e problem_name.cuik/world \e steps \e action
       .
   where
   ...
   \param argc Number of elements in the command line that starts the application (i.e., the
               \b cuiksimulateEuler command itself and its arguments).
...
int main(int argc, char **arg)
{
  ...
  if (argc>3)
	...
	action[i]=atof(arg[3+i]);
      ...
      if (db>0)
	Error("cuiksimulateEuler does not deal with bifurcations (yet)");
 ...
 else
   {
     ...
     fprintf(stderr,"  Use:\n");
     fprintf(stderr,"      cuiksimulateEuler <problem filename>.%s <steps> <action>\n",CS_WD_EXT);
