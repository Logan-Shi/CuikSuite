...
/** \file cuikkinorrt.c
    ...
    \brief Main body of the \ref cuikkinorrt.c "cuikkinorrt" application.
    ...
    digraph applications {
      ...
      cuikkinorrt    [label="cuikkinorrt"  color="#99ff99" shape=ellipse];
    ...
    Defines a kinodynamic RRT on a manifold using the method proposed in
     ...
     - S. LaValle and J. Kuffner, Randomiezed Kynodynamic Planning
        International Journal of Robotics Research, Vol 20, N. 5, 2001.
     .
...
/** 
   ...
   Main body of the \ref cuikkinorrt.c "cuikkinorrt" application.
   ...
   Use:
       - \b cuikkinorrt \e problem_name.world
       .
   where
   ...
   \param argc Number of elements in the command line that starts the application (i.e., the
               \b cuikkinorrt command itself and its arguments).
...
int main(int argc, char **arg)
{
  ...
  if (argc>1)
      ...
      if (argc>2)
	{
	  ...
	  if (nRepetitions==0)
	    Error("Second parameter for cuikkinorrt is wrong");
	}
      else
	nRepetitions=1;
      ...
      for(it=0;it<nRepetitions;it++)
	{
	  ...
	  if (birrt)
	    connected=kinobiRRT(&parameters,intFunction,s2,
				&planningTime,
				&pl,&ns,&path,
				&da,&actions,&times,
				rst,&world,&rrt);
	  else
	    connected=kinoRRT(&parameters,intFunction,s2,
			      &planningTime,
			      &pl,&ns,&path,
			      &da,&actions,&times,
			      rst,&world,&rrt);
  ...
  else
    {
      ...
      fprintf(stderr,"  Use:\n");
      fprintf(stderr,"      cuikkinorrt <problem filename>.%s [num Repetitions]\n",CS_WD_EXT);
